...
//                stu.setHead_image(res.getBytes(2));
    ...
    public void modAct(Activity act) throws BaseException, SQLException {
        ...
        pst.setTimestamp(6, new Timestamp(act.getStartTime().getTime()));
        pst.setTimestamp(7, new Timestamp(act.getEndTime().getTime()));
...
//            pst.execute();
    ...
    public void delAct(int actid) throws SQLException {
        ...
        String sql="update act set state=?,remarks=? where activityId=?";
        PreparedStatement pst=conn.prepareStatement(sql);
        pst.setString(1, "del");
    ...
    public  Activity getActById(int actid){
        ...
        try
        {
            ...
            while (rs.next()) {
                ...
                a.setStartTime(new Date(rs.getTimestamp(7).getTime()));
                a.setEndTime(new Date(rs.getTimestamp(8).getTime()));
    ...
    public List<Activity> listActInAsso() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                else if(a.getStatus().equals("del"));
                else if(a.getStatus().equals("delok"));
                else if(a.getStatus().equals("over"));
                else{
