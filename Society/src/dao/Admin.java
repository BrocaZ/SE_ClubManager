...
//                pst.setString(1, "待举办");
    ...
    public void checkModAct(Activity act,boolean accept) throws BaseException {
        ...
        try
        {
            ...
            if(accept){
                ...
                String sql="UPDATE act set placeId = ?,acttheme = ?,activityContent = ?,leaderSno = ?,start_time = ?,end_time = ?,attend_number = ?,state=?,remarks=? where activityId = ?";
                PreparedStatement pst=conn.prepareStatement(sql);
                ...
                pst.setInt(10,id);
                pst.setString(8,"ok");
                pst.setString(9,act.getRemarks());
                pst.execute();
    ...
    //管理员按审批按钮，可以是管理员收到邮件，点击后显示信息，然后有一个审批按钮
    public void checkDelAct(Activity act,boolean accept) throws BaseException, SQLException {
        Connection conn=null;
        conn=this.getConnection();
        if(!accept){
            String sql="UPDATE act set state = 'ok' where activityId = ?";
...
//                pst.setString(1, "待举办");
            ...
            String sql="UPDATE act set state='delok' where activityId = ?";
            PreparedStatement pst=conn.prepareStatement(sql);
//                pst.setString(1, "待举办");
            pst.setInt(1,act.getActivityId());
            pst.execute();
...
//    }
    ...
    public List<Activity> listActCheck() {
        ...
        try {
            conn = this.getConnection();
            String sql = "select activityId,placeId,associationId,acttheme,activityContent,leaderSno,start_time,end_time,attend_number,state,remarks from act where state != 'ok' and state != 'delok' and state != 'over'";
            PreparedStatement pst = conn.prepareStatement(sql);
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
